...
Then we prep the final dataframes that will make up our tables.
...
We set up tables for each of our results tables.

```{r}

# Create mysql tables; use tryCatch block in case the tables are already created
...
# Add data frames to tables, but first see if the tables have already been
...
From there we can close our connection.

```{r}
#dbRemoveTable(con, "df_cuny")
#dbRemoveTable(con, "df_search")
#dbRemoveTable(con, "df_compare")
#dbRemoveTable(con, "cuny_count")
#dbRemoveTable(con, "search_count")
dbDisconnect(con)
```
